...
class DataSetMetaDataGenerator(object):
    ...
    def create_sections(self, dataset_id, age_sex_combo, migration_arrival_combo):
        first_section_data_elements = self.build_data_elements(
            'First', age_sex_combo)
        second_section_data_elements = self.build_data_elements(
            'Second', migration_arrival_combo)
        section_info = [{'name': 'first section',
                         'elements': first_section_data_elements},
                        {'name': 'second section',
                         'elements': second_section_data_elements}]
        sections = []
    ...
    def run(self):
        ...
        age_sex_combo = self.build_combo(
            'Some Age and Sex', age_sex_categories, category_options)
        migration_arrival_categories, migration_category_options = self.build_migration_arrival_categories()
        migration_arrival_combo = self.build_combo(
            'Migration and Arrival', migration_arrival_categories, migration_category_options)
        sections, data_elements = self.create_sections(
            datasets[0]['id'], age_sex_combo, migration_arrival_combo)
...
def main():
    ...
    username = 'admin'
    password = 'district'
    user_data_generator = UserMetaDataGenerator(
        'users.csv', url, username, password)
    datasetgenerator = DataSetMetaDataGenerator(
        url, username, password, user_data_generator)
